...
type AdminTab = "overview" | "users" | "activity" | "feedback" | "audit" | "emails" | "system" | "analytics";
...
export default function AdminPanel() {
  ...
  const loadAdminData = async (isRefresh = false) => {
    ...
    try {
      ...
      const [overviewRes, usersRes, activityRes, feedbackRes, auditRes, emailRes, analyticsRes] = await Promise.all([
        authFetch(`${API_BASE}/admin/overview`),
        authFetch(`${API_BASE}/admin/users?${params.toString()}`),
        authFetch(`${API_BASE}/admin/activity?${activityParams.toString()}`),
      ...
      <main className="mx-auto max-w-[1440px] px-4 py-6 sm:px-6 lg:px-8">
        ...
        {tab === "users" && (
          <div className="space-y-4">
            ...
            {users.length ? (
              <UserTable users={users} onSelect={setSelectedUser} onAction={adminAction} actionLoading={actionLoading} />
            ) : (
...
function ActivityList({ activity, full = false }: { activity: AdminActivity[]; full?: boolean }) {
  if (!activity.length) {
    return <EmptyState title="No logs found" body="Activity logs will appear after scans, exports, context builds, and memory updates." />;
  }
...
function UserDetailPanel({ user: u, onClose, onAction, actionLoading }: { user: AdminUser; onClose: () => void; onAction: Function; actionLoading: string | null }) {
...
function UserTable({ users, onSelect, onAction, actionLoading }: { users: AdminUser[]; onSelect: (u: AdminUser) => void; onAction: Function; actionLoading: string | null }) {
  return (
    <div className="overflow-hidden rounded-2xl border border-white/[0.06]">
      <div className="overflow-x-auto">
        <table className="w-full min-w-[1100px] text-left">
          ...
          <tbody className="divide-y divide-white/[0.05] bg-[#0d0f1a]/70">
            {users.map((item) => (
              <tr key={item.id} className="align-top hover:bg-white/[0.02] cursor-pointer" onClick={() => onSelect(item)}>
