...
export class ArchitectureGuard {
    ...
    private validateTypeScript(content: string, filePath: string): vscode.Diagnostic[] {
        ...
        while ((match = catchRegex.exec(content)) !== null) {
            ...
            diagnostic.code = 'AI-TS002';
            diagnostic.source = 'AI Context Brain';
            diagnostics.push(diagnostic);
        }

        // Check for direct state mutation in React
        if (filePath.includes('React') || content.includes('React')) {
            const stateMutationRegex = /this\.state\.[a-zA-Z_][a-zA-Z0-9_]*\s*=/g;
            while ((match = stateMutationRegex.exec(content)) !== null) {
                const position = this.getPosition(content, match.index);
                const range = new vscode.Range(position, position.translate(0, match[0].length));
                const diagnostic = new vscode.Diagnostic(
                    range,
                    "Use setState instead of direct state mutation",
                    vscode.DiagnosticSeverity.Warning
                );
                diagnostic.code = 'AI-REACT001';
                diagnostic.source = 'AI Context Brain';
                diagnostics.push(diagnostic);
            }
        }
    ...
    private async performBackendValidation(projectPath: string, filePath: string, _content: string): Promise<vscode.Diagnostic[]> {
        const diagnostics: vscode.Diagnostic[] = [];

        try {
            const result = await this.apiClient.validateArchitecture(projectPath, filePath);
