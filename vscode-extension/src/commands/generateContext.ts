...
export class GenerateContextCommand {
    ...
    async generateAndWrite(projectPath: string, maxTokens?: number, contextType: string = 'full', openFile: boolean = false): Promise<void> {
        ...
        await vscode.window.withProgress({
            ...
            progress.report({ increment: 15, message: 'Generating optimized context...' });
            const result = this.apiClient
                ? await generateOptimizedContext(this.apiClient, projectPath, tokenLimit, contextType)
                : {
