...
import { analyzeCodeIntelligence, AnalysisSourceFile, extractGitDecisions } from '../services/codeIntelligenceAnalyzer';
...
export class ScanProjectCommand {
                ...
                try {
                    await this.apiClient.scanProject(projectPath, {
                        ...
                        Metrics: {
                            ...
                            DecisionMap: localResult.metrics.decisionMap,
    ...
    private async analyzeLocally(projectPath: string): Promise<any> {
        ...
        const analysisSourceFiles: AnalysisSourceFile[] = [];
        ...
        for (const file of filesToDeepAnalyze) {
            try {
                ...
                const content = new TextDecoder('utf-8').decode(data);
                const relPath = path.relative(projectPath, file.fsPath).replace(/\\/g, '/');
                analysisSourceFiles.push({ path: relPath, content });
        ...
        // Structural code intelligence supersedes broad regex maps when it has
        // source-backed results and enriches modules/decisions independently.
        const intelligence = analyzeCodeIntelligence(analysisSourceFiles);
        intelligence.decisionMap.push(...await extractGitDecisions(projectPath));
        if (intelligence.routeMap.length > 0) routeMap.splice(0, routeMap.length, ...intelligence.routeMap);
        if (intelligence.serviceGraph.length > 0) serviceGraph.splice(0, serviceGraph.length, ...intelligence.serviceGraph);
        if (intelligence.entityMap.length > 0) entityMap.splice(0, entityMap.length, ...intelligence.entityMap);
        if (intelligence.aiProviderMap.length > 0) aiProviderMap.splice(0, aiProviderMap.length, ...intelligence.aiProviderMap);
        for (const detectedModule of intelligence.moduleMap) {
            const existingIndex = moduleMap.findIndex(module => module.name === detectedModule.name);
            if (existingIndex >= 0) moduleMap[existingIndex] = { ...moduleMap[existingIndex], ...detectedModule };
            else moduleMap.push(detectedModule);
        }
        ...
        return {
            ...
            metrics: {
                ...
                decisionMap: intelligence.decisionMap.length > 0 ? intelligence.decisionMap : undefined,
